...
criterion = nn.CrossEntropyLoss(); optimizer = torch.optim.Adam(model_root.parameters(), lr=lr_root)
...
if train_root:
    ...
    for epoch_ in range(epoch_root):
        ...
        if True:
            model_root.eval()
            ACC_, ASR_ = utils_attack.test_asr_acc_ISSBA(dl_te=dl_te, model=model_root, label_backdoor=label_backdoor,
                                            secret=secret, encoder=encoder_issba, device=device)
    ...
    # model_root.load_state_dict(torch.load(exp_dir+'/'+f'root_model_13.pth'))
    # ACC_, ASR_ = utils_attack.test_asr_acc_ISSBA(dl_te=dl_te, model=model_root, label_backdoor=label_backdoor,
    #                                         secret=secret, encoder=encoder_issba, device=device)
    model_root.eval(); model_root.requires_grad_(False)
    loss_fn_alex = lpips.LPIPS(net='alex').cuda()
    train_B = False 
    if train_B:
        for epoch_ in range(epoch_B):
            loss_sum = 0.0; loss_wass_sum = 0.0; loss_lpips_sum = 0.0
            for i in range(max(len(dl_root), len(dl_sus))):
                ...
                # loss_wass_root = utils_defence.wasserstein_distance(model_root(B_root), model_root(X_root))
                
                loss = los_mse + lpips_loss_op.mean()+loss_wass 
                loss.backward()
                optimizer_B.step()
                loss_sum+=loss.item()
                loss_wass_sum+=loss_wass.item()
                loss_lpips_sum+=lpips_loss_op.mean().item()
            print(f'epoch: {epoch_}, loss: {loss_sum/len(dl_sus): .2f}')
            print(f'loss wass : {loss_wass_sum/len(dl_sus): .2f}')
            print(f'loss lpips : {loss_lpips_sum/len(dl_sus): .2f}')
        ...
        for index in [100, 200, 300, 400, 500, 600]:
            with torch.no_grad(): 
                ...
                issba_image = encoded_image.squeeze().cpu().detach().numpy().transpose((1, 2, 0))
                plt.imshow(issba_image)
                plt.savefig(exp_dir+f'/generated2_{index}.pdf')
