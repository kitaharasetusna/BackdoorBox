...
with torch.no_grad():
    for images, labels in dl_te:
        ...
        for xx in range(len(mali_images)):
            mali_images[xx] = utils_attack.add_blended_trigger(inputs=mali_images[xx], pattern=pattern, alpha=0.2)
...
with torch.no_grad():
    for images, labels in dl_te:
        ...
        for xx in range(len(images)):
            images[xx] = utils_attack.add_blended_trigger(inputs=images[xx], pattern=pattern.to(device), alpha=0.2)
        outputs = model(images)
        _, predicted = torch.max(outputs, 1)
