...
ds_p = Subset(ds_whole_poisoned, ids_p)
dl_p =  DataLoader(dataset= ds_p,batch_size=bs_tr,shuffle=True,num_workers=0,drop_last=True)
loader_root_iter = iter(dl_root); loader_sus_iter = iter(dl_sus) 
load_poisoned_iter = iter(dl_p)
...
if train_B:
    for epoch_ in range(epoch_B):
        ...
        print(f'loss mse: {loss_mse_sum/len(dl_p): .2f}')
        print(f'loss logits: {loss_logits_sum/len(dl_p): .2f}')
        print(f'loss inf: {loss_inf_sum/len(dl_p): .2f}')
        if (epoch_+1)%5==0 or epoch_==epoch_B-1 or epoch_==0:
            ...
            torch.save(B_theta.state_dict(), exp_dir+'/'+f'step3_B_theta_mali_{epoch_+1}.pth')
else:
    pth_path = exp_dir+'/'+f'step3_B_theta_mali_{15}.pth'
    ...
    for index in [100, 200, 300, 400, 500, 600]:
        with torch.no_grad(): 
            ...
            plt.savefig(exp_dir+f'/ISSBA_whole_{index}.pdf')

            encoded_image = B_theta(image)
            encoded_image = utils_data.unnormalize(encoded_image, mean=[0.4914, 0.4822, 0.4465], std=[0.247, 0.243, 0.261]) 
            issba_image = encoded_image.squeeze().cpu().detach().numpy().transpose((1, 2, 0))
            plt.imshow(issba_image)
            plt.savefig(exp_dir+f'/generated_whole_{index}.pdf')
