...
bs_tr = 128
epoch_B = 10; lr_B = 1e-4; lr_ft = 1e-3
...
dl_sus = DataLoader(dataset= ds_whole_poisoned,batch_size=bs_tr,shuffle=True,num_workers=0,drop_last=True)
...
if train_B:
    for epoch_ in range(epoch_B):
        ...
        for i in range(max(len(dl_root), len(dl_sus))):
            ...
            loss.backward()
            optimizer.step()
            loss_sum+=loss.item(); loss_mse_sum+=los_mse.item(); loss_wass_sum+=los_logits.item()
        print(f'epoch: {epoch_}, loss: {loss_sum/len(dl_sus): .2f}')
        print(f'loss mse: {loss_mse_sum/len(dl_sus): .2f}')
        print(f'loss logits: {loss_wass_sum/len(dl_sus): .2f}')
