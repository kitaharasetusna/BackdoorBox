...
dl_q =  DataLoader(dataset= ds_whole_poisoned,batch_size=bs_tr,shuffle=True,num_workers=0,drop_last=True)
loader_root_iter = iter(dl_root); loader_sus_iter = iter(dl_sus) 
loader_question_iter = iter(dl_q)
...
if train_B:
    for epoch_ in range(epoch_B):
        ...
        print(f'loss mse: {loss_mse_sum/len(dl_q): .2f}')
        print(f'loss logits: {loss_logits_sum/len(dl_q): .2f}')
        print(f'loss inf: {loss_inf_sum/len(dl_q): .2f}')
        if (epoch_+1)%5==0 or epoch_==epoch_B-1 or epoch_==0:
            ...
            torch.save(B_theta.state_dict(), exp_dir+'/'+f'step3_B_theta_whole_{epoch_+1}.pth')
else:
    pth_path = exp_dir+'/'+f'step3_B_theta_whole_{1}.pth'
