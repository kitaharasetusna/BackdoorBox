...
# TODO: add normalization when you train B_\theta !!!(for both training and test gen)
# TODO: change this for other attacks (after ISSBA is verified to work)
...
# ----------------------------------------- configs:
exp_dir = f'../experiments/exp8/{args.attack}' 
if args.create_configs:
    print('creating configuration yaml files')
    os.makedirs(exp_dir, exist_ok=True) 
    with open(exp_dir+'/config.yaml', 'w') as yaml_file:
    ...
    configs = utils_load.load_config(exp_dir+'/config.yaml')
    dataset = configs['dataset']
    # configs_attack
    label_backdoor = configs['label_backdoor']; secret_size = configs['secret_size']
    bs_te = configs['bs_te']
