...
# ----------------------------------------- 1 train B_theta  
# prepare B
ds_whole_poisoned = utils_attack.CustomCIFAR10ISSBA_whole(ds_tr,
                    ids_p, label_backdoor, secret=secret, encoder=encoder_issba,
                    device=device)
...
if train_B:
    ...
    for index in [100, 200, 300, 400, 500, 600]:
        with torch.no_grad(): 
            ...
            encoded_image = utils_attack.add_ISSBA_trigger(inputs=image_c, secret=secret,
    ...
    utils_attack.fine_tune_ISSBA(dl_root=dl_root, model=model, label_backdoor=label_backdoor,
                                 B=B_theta, device=device, dl_te=dl_te, secret=secret,
                                 optimizer=optimizer, criterion=criterion, encoder=encoder_issba, epoch=10)
