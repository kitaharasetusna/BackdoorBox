...
if train_B:
    ...
    for epoch_ in range(epoch_B):
        ...
        if (epoch_+1)%5==0 or epoch_==epoch_B-1 or epoch_==0:
            utils_attack.test_asr_acc_BATT_gen2(dl_te=dl_te, model=model, label_backdoor=label_backdoor,
                                                B=B_theta, device=device)
            torch.save(B_theta.state_dict(), exp_dir+'/'+f'B_theta_non_ts_{epoch_+1}.pth')
else:
    pth_path = exp_dir+'/'+f'B_theta_non_ts_{20}.pth'
    ...
    for index in [100, 200, 300, 400, 500, 600]:
        with torch.no_grad(): 
            ...
            issba_image = encoded_image.squeeze().cpu().detach().numpy().transpose((1, 2, 0))
            plt.imshow(issba_image)
            plt.savefig(exp_dir+f'/Gen_non_TS_{index}.pdf') 
