...
with torch.no_grad():
    for images, labels in dl_te:
        ...
        for xx in range(len(gen_images)):
            gen_images[xx] = utils_attack.add_ISSBA_gen(inputs=gen_images[xx].unsqueeze(0), 
                                                        B=B_theta, device=device) 
        for xx in range(len(gen_images_ori)):
            gen_images_ori[xx] = utils_attack.add_ISSBA_gen(inputs=gen_images_ori[xx].unsqueeze(0), 
                                                        B=B_theta_ori, device=device) 
        for xx in range(len(mali_images)):
            mali_images[xx] = utils_attack.add_batt_trigger(inputs=mali_images[xx], rotation=rotation)
...
with torch.no_grad():
    for images, labels in dl_te:
        ...
        for xx in range(len(images)):
            images[xx] = utils_attack.add_batt_trigger(inputs=images[xx], rotation=rotation)
        outputs = model(images)
        _, predicted = torch.max(outputs, 1)
