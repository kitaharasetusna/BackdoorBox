...
for epoch_ in range(epoch_root):
    for inputs, targets in dl_root:
        inputs_gen, output_gen = copy.deepcopy(inputs), copy.deepcopy(targets)
        for xx in range(len(inputs_gen)):
            inputs_gen[xx] = utils_attack.add_ISSBA_gen(inputs=inputs_gen[xx], B=B_theta, device=device)
            output_gen[xx] = label_backdoor
        
        inputs = torch.cat((inputs, inputs_gen), dim=0)
        targets = torch.cat((targets, output_gen))
    ...
    if True:
        ...
        with open(exp_dir+f'/root_model_acc_asr.pkl', 'wb') as f:
