...
print(ACC_, ASR_)
if train_B:
    with open(exp_dir+f'/idx_suspicious2_{ratio}.pkl', 'rb') as f:
        idx_sus = pickle.load(f)
        print(len(idx_sus))
else:
    with open(exp_dir+f'/idx_suspicious2.pkl', 'rb') as f:
        idx_sus = pickle.load(f)
...
if train_B:
    ...
    utils_attack.fine_tune_Wanet2(dl_root=dl_root, model=model, label_backdoor=label_backdoor,
                                B=B_theta, device=device, dl_te=dl_te, identity_gird=identity_grid,
                                noise_grid=noise_grid,
